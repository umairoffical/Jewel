(function($){
    ...
    if ( typeof HOMEY_map_vars !== "undefined" ) {
        ...
        var paged = 0;
        var compare_url = HOMEY_map_vars.compare_url;
        var add_compare = HOMEY_map_vars.add_compare;
        var remove_compare = HOMEY_map_vars.remove_compare;
        var compare_limit = HOMEY_map_vars.compare_limit;
        ...
        *---------------------------------------------------------------------------*/
        ...
        var homey_submit_autocomplete = function() {
            ...
            jQuery('#listing_address, #experience_address').autocomplete( {
                source: function ( request, response ) {
                    // Always search within Los Angeles area
                    var searchTerm = request.term + ', Los Angeles, CA';
                    
                    jQuery.get( 'https://nominatim.openstreetmap.org/search', {
                        format: 'json',
                        q: searchTerm,
                        addressdetails: '1',
                        // Expanded viewbox to cover all of Los Angeles County
                        viewbox: '-118.9517,34.8233,-117.6462,33.7037', // LA County boundaries
                        bounded: 1,
                        ...
                        if ( !result.length ) {
                            response( [ {
                                value: '',
                                label: 'No results found in Los Angeles area'
                            } ] );
                            return;
                        }

                        // Filter results to include all Los Angeles County addresses
                        var laResults = result.filter(function(item) {
                            return (item.address.county === 'Los Angeles County' || 
                                   item.address.city === 'Los Angeles' ||
                                   (item.address.state === 'California' && 
                                    parseFloat(item.lat) >= 33.7037 && 
                                    parseFloat(item.lat) <= 34.8233 &&
                                    parseFloat(item.lon) >= -118.9517 && 
                                    parseFloat(item.lon) <= -117.6462));
                        });
                        ...
                        response( laResults.map( function ( item ) {
                            var return_obj = {
                                label: item.display_name,
                                latitude: item.lat,
                                longitude: item.lon,
                            ...
                            if(typeof(item.address) != 'undefined') {
                                return_obj.county = item.address.county || 'Los Angeles County';
                                return_obj.city = item.address.city || 'Los Angeles';
                                return_obj.state = 'California';
                                return_obj.country = 'United States';
                                return_obj.zip = item.address.postcode;
                                return_obj.country_short = 'US';
                            }
